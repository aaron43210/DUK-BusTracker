...
from services.valhalla_client import get_valhalla_distance_m, get_valhalla_distance_matrix_m, reset_snap_state
...
async def handle_power_off(db: AsyncSession, manager, now_utc: datetime) -> None:
    ...
    if trip and trip.status in ("on_trip", "late"):
        ...
        if last_log:
            ...
            dist = await get_valhalla_distance_m(
...
async def handle_gps_update(
    ...
    if trip.status == "scheduled":
        ...
        dist_from_start = await get_valhalla_distance_m(start_lat, start_lon, lat, lon)
    ...
    # ── on_trip / late → proximity checks ────────────────────────────────────
        ...
        dest_lat, dest_lon = await get_destination_coords(db, trip.direction)
        dist_to_dest = await get_valhalla_distance_m(lat, lon, dest_lat, dest_lon)
        logger.debug(
        ...
        try:
            ...
            if candidates:
                ...
                distances = await get_valhalla_distance_matrix_m(lat, lon, dests)
                new_ids   = [
