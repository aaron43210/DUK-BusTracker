...
from services.valhalla_client import get_valhalla_route_geometry, trace_valhalla_path
...
async def _apply_map_matching(
    ...
    """
    Applies high-fidelity map matching using Valhalla.
    """
    if len(raw_points) < 2:
        return raw_points
    ...
    for chunk in chunks:
        coords_tuple = [(pt["lat"], pt["lon"]) for pt in chunk]
        
        try:
            # Use trace_valhalla_path (map matching) instead of simple routing
            route_coords = await trace_valhalla_path(coords_tuple)
