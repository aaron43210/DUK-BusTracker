...
import { fetchValhallaRoute } from '../utils/valhalla';
...
export default function MapFull() {
  ...
  const fetchAll = useCallback(async () => {
    try {
      ...
      const currentStops = fetchedStops.length > 0 ? fetchedStops : stops;
      ...
      if (plannedCoords.length === 0 && currentStops.length >= 2) {
        try {
          const coords = currentStops.map(s => [Number(s.lon), Number(s.lat)]);
          const valhallaRoute = await fetchValhallaRoute(coords);
          
          if (valhallaRoute && valhallaRoute.length > 0) {
            setPlannedCoords(valhallaRoute);
          } else {
            // Fallback to static geometry if Valhalla API fails
